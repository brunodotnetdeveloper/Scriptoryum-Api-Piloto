...
public class AIConfigController : ControllerBase
{
    ...
    /// <summary>
    /// Testa uma API key para um provedor específico
    /// </summary>
    /// <param name="testDto">Dados para teste da API key</param>
    /// <returns>Resultado do teste</returns>
    [HttpPost("test-key")]
    [SwaggerOperation(
        Summary = "Testar API key",
        Description = "Testa se uma API key é válida e funcional para um provedor específico"
    )]
    [SwaggerResponse(200, "Teste realizado com sucesso", typeof(TestApiKeyResponseDto))]
    [SwaggerResponse(400, "Dados inválidos", typeof(TestApiKeyResponseDto))]
    [SwaggerResponse(401, "Não autorizado")]
    [SwaggerResponse(500, "Erro interno do servidor", typeof(TestApiKeyResponseDto))]
    public async Task<ActionResult<TestApiKeyResponseDto>> TestApiKey([FromBody] TestApiKeyDto testDto)
    {
        try
        {
            if (!ModelState.IsValid)
            {
                ...
                return BadRequest(new TestApiKeyResponseDto
                {
                    Success = false,
                    Message = "Dados inválidos",
                    Errors = errors
            ...
            var result = await _aiConfigService.TestApiKeyAsync(testDto);
            
            // Sempre retorna 200 OK, o resultado do teste está no DTO
            return Ok(result);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Erro ao testar API key para provedor {Provider}", testDto.Provider);
            return StatusCode(500, new TestApiKeyResponseDto
            {
                Success = false,
                Message = "Erro interno do servidor",
                Errors = new List<string> { "Ocorreu um erro inesperado" }
